...
## Find the most abundant family in a version of RefSeq
## Example usage:
## ./find_most_abundant_family.pl taxonomy/names.txt taxonomy/nodes.dmp catalogs/RefSeq-release1.catalog.gz
...
sub get_species
{
    ...
    if (exists $Type{$s}) {
	my $type = $Type{$s};
	while($type ne 'family' && $type ne 'order'  && $type ne 'class'  && $type ne 'phylum'  && $type ne 'kingdom'  && $type ne 'superkingdom' ) {
	    $s = $Parent{$s};
	    $type = $Type{$s};
	}
	$species = $Names{$s};
